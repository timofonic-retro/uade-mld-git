	*****************************************************
	****    Jochen Hippel 7V 68020+ replayer for     ****
...
HIGHEST_MIXING_RATE	equ	29
	...
	dc.b	'$VER: Jochen Hippel 7V player module V1.3 [68020+] (17 May 2014)',0
	even
Tags
...
NoMemory
	clr.l	OtherSize(A4)
	move.l	SamplesPtr(PC),(A2)		; for relocation
Skipper
	movea.l	dtg_AudioAlloc(A5),A0
	jmp	(A0)

HigherCPU
	moveq	#EPR_NeedhigherCPUFPU,D0
	rts
...
EndPlayer
	move.l	InfoBuffer+OtherSize(PC),D0
	beq.b	SkipFree
...
InterruptStruct
	...
	dc.l	Interrupt
Name
	dc.b	'Jochen Hippel 7V Aud0 Interrupt',0,0
	even
...
Interrupt
	...
	move.w	Period4(PC),UPS_Voice1Per(A0)
...
;	ADD.L	D6,D0			; 6
;	ADDX.W	D4,D0			; 4
;	BPL.S	lbC000F1E		; 8/10
;lbC000F02	ADD.L	D7,D1		; 6
;	ADDX.W	D4,D1			; 4
;	BPL.S	lbC000F28		; 8/10
;lbC000F08	ADD.L	A5,D2		; 6
;	ADDX.W	D4,D2			; 4
;	BPL.S	lbC000E92		; 8/10
;lbC000F0E	ADD.L	A6,D3		; 6
;	ADDX.W	D4,D3			; 4
;	BPL.S	lbC000E9C		; 8/10
...
;	MOVE.W	lbW007C0A(PC),$DFF0D4
	...
	move.w	#$80,$DFF09A
	move.w	#$80,$DFF09C
	move.l	lbL007620(PC),$DFF0A0
	move.w	lbW007C0A(PC),$DFF0A4

	move.l	lbL007620(PC),StructAdr+UPS_Voice1Adr
	move.w	lbW007C0A(PC),StructAdr+UPS_Voice1Len
...
;	BSR.L	lbC000EAE
	...
	move.l	lbL000E86(PC),D4
	move.l	lbL000E8A(PC),-(SP)
	bra.w	Top
...
calc1					; 68030
	swap	D5			; 0 (12/24) to data
	clr.w	D5			; 0 (14/24)
	add.l	A5,D0			; 0 (16/24)
	addx.w	D5,D0			; 0 (18/24)
	bpl.s	clvc1a			; 0 (24/24) 6/8
cbk1a
	add.l	A6,D1			; 2
	addx.w	D5,D1			; 2
	bpl.s	clvc2a			; 6/8
...
cbk3a
	...
	bpl.s	clvc4a			; 6/8
Top
	move.b	(A1,D1.W),D6		; 12 data 2
	move.w	v7contab(PC,D6.W*2),D5	; 12 volume 2
	swap	D6			; 4
	move.b	(A0,D0.W),D6		; 12 data 1
	add.w	v7contab(PC,D6.W*2),D5	; 12 mix 1 in 2
	move.b	(A3,D3.W),D7		; 12 data 4
	add.w	v7contab(PC,D7.W*2),D5	; 12 mix 4 in 1/2
	swap	D7			; 4
	move.b	(A2,D2.W),D7		; 12 data 3
	add.w	v7contab(PC,D7.W*2),D5	; 12 mix 3 in 1/2/4
	move.b	v7contab(PC,D5.W*2),(A4)+	; 16+28 mixed byte in buffer
	clr.w	D5			; 0 (2/24)
	add.l	A5,D0			; 0 (4/24)
	addx.w	D5,D0			; 0 (6/24)
	bpl.s	clvc1			; 0 (12/24) 6/8
cbk1
	add.l	A6,D1			; 0 (14/24)
	addx.w	D5,D1			; 0 (16/24)
	bpl.s	clvc2			; 0 (22/24) 6/8
cbk2
	add.l	D4,D2			; 0 (24/24)
	addx.w	D5,D2			; 2
	bpl.s	clvc3			; 6/8
...
cbk4
	move.b	(A0,D0.W),D6		; 12 data 1
	move.w	v7contab(PC,D6.W*2),D5	; 12 volume 1
	swap	D6			; 4
	move.b	(A1,D1.W),D6		; 12 data 2
	add.w	v7contab(PC,D6.W*2),D5	; 12 mix 2 in 1
	move.b	(A2,D2.W),D7		; 12 data 3
	add.w	v7contab(PC,D7.W*2),D5	; 12 mix 3 in 1/2
	swap	D7			; 4
	move.b	(A3,D3.W),D7		; 12 data 4
	add.w	v7contab(PC,D7.W*2),D5	; 12 mix 4 in 1/2/3
	move.b	v7contab(PC,D5.W*2),(A4)+	; 16+28 mixed byte in buffer
	swap	D5			; 0 (2/24) to dbf
	dbf	D5,calc1		; 0 (8/24)
	bra.w	clcback			; total minimum 356 cycles (51 commands)
					; total minimum 178 cycles per mixed byte
			; original version total minimum 268 cycles (32 commands)
...
lbL007844	ds.b	480+120			; extended buffy

lbL007A24	ds.b	480+120			; extended buffy
